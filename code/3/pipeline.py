    ...
    for file_name in os.listdir(input_folder):
        ...
        for protected_attribute in protected_attribute_list:
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(skf.split(data, strat_labels)):
                ...
                # --- outlier (TypologyDetector) exploratory detection DISABLED ---
                # This code/3 pipeline targets the subgroup-mitigation methods
                # (borrow / undersample / threshold); outlier detection is left off
                # on purpose so the full run is focused and fast.
...
input_folder_name = "3"          # datasets/inputs/3/  (contains 3.csv)
method_number = "3"
...
# =====================================================================
# SUBGROUP-MITIGATION METHOD SELECTOR (the code/3 experiment)
# ---------------------------------------------------------------------
# Pick ONE synthesis method; it wires the env-var toggles read at runtime by
# generate_samples.new_apply. Run once per method (each writes to its own
# output folder), then compare. Privacy/fairness/utility land in the usual
# results_metrics/{linkability,fairness}_results/outputs_4/<final_folder>/.
#
#   "baseline"      -> unchanged FairPrivSMOTE (control)
#   "borrow"        -> cross-subgroup neighbor borrowing
#   "undersample"   -> hybrid balancing (downsample large subgroups)
#
# "borrow + threshold" is NOT a separate synthesis method: run method="borrow"
# here, then apply the per-group DECISION threshold as a post-processing step:
#       priv39/bin/python code/3/apply_group_threshold.py 3_borrow
# (the threshold fixes DI/SPD at decision time; it does not change synthesis).
#
# NOTE: FPS_BORROW_THRESHOLD below is a SUBGROUP-SIZE gate (borrow only fires for
# subgroups smaller than this many rows). It is unrelated to the decision
# threshold used by apply_group_threshold.py.
# =====================================================================
method = "undersample"   # <-- change to "baseline" / "undersample"

# clear all toggles first so a run is reproducible regardless of shell state
for _k in ("FPS_BORROW_NEIGHBORS", "FPS_BORROW_THRESHOLD",
           "FPS_UNDERSAMPLE", "FPS_UNDERSAMPLE_TARGET", "FPS_UNDERSAMPLE_QUANTILE",
           "FPS_CAP_MULTIPLE", "FPS_CAP_FACTOR"):
    os.environ.pop(_k, None)

if method == "borrow":
    os.environ["FPS_BORROW_NEIGHBORS"] = "1"
    os.environ["FPS_BORROW_THRESHOLD"] = "25"   # subgroup-SIZE gate: borrow if < 25 rows
elif method == "undersample":
    os.environ["FPS_UNDERSAMPLE"] = "1"
    os.environ["FPS_UNDERSAMPLE_TARGET"] = "120"  # downsample every subgroup to <=120
elif method != "baseline":
    raise ValueError(f"unknown method: {method!r} (use 'baseline', 'borrow', or 'undersample')")

final_folder_name = f"3_{method}"   # e.g. results land under .../outputs_4/3_borrow/
print(f"[code/3] running method='{method}' -> final_folder_name='{final_folder_name}'")
