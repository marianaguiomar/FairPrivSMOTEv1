...
# --- path setup for running this copy from code/small_disjuncts_pipeline/ ---
# This dir is sys.path[0] (so the local pipeline_helper copy resolves). We also add code/
# (for the package imports main.*, metrics.*, others.*, outliers.*, helpers.*) and code/main
# (so the original algorithm modules' bare imports, e.g. `from generate_samples import ...`,
# still resolve to the originals -- we reuse the unchanged FP-SMOTE core, only the driver changes).
...
final_folder_name = "small_disjuncts"    # output group: datasets/outputs/outputs_4/small_disjuncts
method_number = "3"
...
### FP-SMOTE generation for the small-disjunct impact analysis ###
