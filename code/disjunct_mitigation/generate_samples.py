...
try:
    import umap  # only needed by plot_fairness_umap; optional
except ImportError:
    umap = None
...
def _enn_clean_synthetic(syn_df, real_df, class_column, k):
    """Wilson-ENN style cleaning of synthetic small-disjunct rows (mitigation #2).

    Drop each synthetic row whose class label disagrees with the MAJORITY class of
    its k nearest REAL neighbours in standardized feature space. This removes the
    overlap-region synthetic points that nudge AOD/EOD the wrong way when we densify
    a small-disjunct pocket. Gated by FPS_DISJUNCT_CLEAN. Returns a filtered copy.
    """
    if syn_df is None or len(syn_df) == 0 or len(real_df) < (k + 1):
        return syn_df
    drop_cols = [class_column, 'single_out', 'small_disjunct']
    feat_cols = [c for c in real_df.columns if c not in drop_cols and c in syn_df.columns]
    if not feat_cols:
        return syn_df
    nr = len(real_df)
    # one-hot encode jointly so string categoricals (e.g. Adult occupation) become numeric
    combined = pd.get_dummies(
        pd.concat([real_df[feat_cols], syn_df[feat_cols]], ignore_index=True),
        dummy_na=False)
    Xall = combined.values.astype(float)
    Xr, Xs = Xall[:nr], Xall[nr:]
    yr = real_df[class_column].values
    mu = Xr.mean(axis=0); sd = Xr.std(axis=0); sd[sd == 0] = 1.0
    Xr = (Xr - mu) / sd; Xs = (Xs - mu) / sd
    nn = NN(n_neighbors=min(k, len(Xr))).fit(Xr)
    idx = nn.kneighbors(Xs, return_distance=False)
    ys = syn_df[class_column].values
    keep = np.empty(len(Xs), dtype=bool)
    for i in range(len(Xs)):
        vals, counts = np.unique(yr[idx[i]], return_counts=True)
        keep[i] = (vals[np.argmax(counts)] == ys[i])
    return syn_df.iloc[keep].reset_index(drop=True)


def _borderline_seed_mask(subset_df, real_df, class_column, k):
    """Borderline-SMOTE DANGER mask (mitigation #1).

    For each row of `subset_df` flagged small_disjunct, look at its k nearest REAL
    neighbours in the WHOLE training set (one-hot feature space, self excluded) and
    keep it as a synthesis seed only if it is on the class boundary: a majority of
    those neighbours are the OPPOSITE class but not ALL of them (k/2 <= opp < k).
    All-opposite = noise (dropped); mostly-same = safe interior (dropped). Returns a
    0/1 Series aligned to subset_df. Gated by FPS_DISJUNCT_BORDERLINE.
    """
    mask = subset_df['small_disjunct'].astype(int).copy()
    sd_idx = subset_df.index[subset_df['small_disjunct'] == 1]
    if len(sd_idx) == 0 or len(real_df) < (k + 1):
        return mask
    drop_cols = [class_column, 'single_out', 'small_disjunct']
    feat_cols = [c for c in real_df.columns if c not in drop_cols and c in subset_df.columns]
    if not feat_cols:
        return mask
    nr = len(real_df)
    combined = pd.get_dummies(
        pd.concat([real_df[feat_cols], subset_df.loc[sd_idx, feat_cols]], ignore_index=True),
        dummy_na=False)
    Xall = combined.values.astype(float)
    Xr, Xs = Xall[:nr], Xall[nr:]
    yr = real_df[class_column].values
    mu = Xr.mean(axis=0); sd = Xr.std(axis=0); sd[sd == 0] = 1.0
    Xr = (Xr - mu) / sd; Xs = (Xs - mu) / sd
    nn = NN(n_neighbors=min(k + 1, len(Xr))).fit(Xr)
    idx = nn.kneighbors(Xs, return_distance=False)[:, 1:]  # drop self (seeds are in real_df)
    ys = subset_df.loc[sd_idx, class_column].values
    for j, ridx in enumerate(sd_idx):
        labs = yr[idx[j]]
        opp = int(np.sum(labs != ys[j]))
        m = len(labs)
        if not (m / 2 <= opp < m):       # not DANGER -> drop from seed set
            mask.loc[ridx] = 0
    return mask
...
def remove_tomek_links(df, class_column, protected_attribute, majority_class, removal_strategy="majority_only", extra_rules=None):
    ...
    # Separate features
    X = pd.get_dummies(
        df.drop(columns=[class_column, protected_attribute, 'single_out', 'synthetic', 'small_disjunct'], errors='ignore'),
        drop_first=True
    ).values
...
def new_apply(dataset, dataset_name, protected_attribute, epsilon, class_column, key_vars, augmentation_rate, k, knn, removal_strategy="majority_only", extra_rules=None, majority=True, binning=None, fold_cache=None, debug_binned_path=None, output_folder=None, qi_only_visualization=False, small_disjunct_mask=None):
    ...
    # === DISJUNCT-AWARE CHANGE: attach the per-row small-disjunct flag ===
    # small_disjunct_mask is a boolean/int array aligned BY POSITION to `dataset`
    # (the training fold passed in). It is computed by a fully-grown decision tree
    # fit on this exact fold (see pipeline.py / disjuncts_textbook/detector.py).
    # We attach it BEFORE the optional undersample step so it travels with the rows.
    # When the mask is absent the column is all-zero and the mitigation is inert.
    if small_disjunct_mask is not None:
        dataset['small_disjunct'] = np.asarray(small_disjunct_mask).astype(int)
    else:
        dataset['small_disjunct'] = 0

    # --- Optional: undersample large subgroups (hybrid balancing) -------------
    # Gated by FPS_UNDERSAMPLE. Randomly downsample every (class,protected)
    # subgroup above a target size, which shrinks the majority count that drives
    # the per-subgroup oversampling target -> the tiny subgroup needs far less
    # synthesis (smaller blow-up, less degenerate blob). Target = absolute
    # FPS_UNDERSAMPLE_TARGET, else the FPS_UNDERSAMPLE_QUANTILE (default 0.5) of
    # subgroup sizes. Never goes below knn+1.
    if os.environ.get("FPS_UNDERSAMPLE", "0") == "1":
        counts_tmp = dataset.groupby([class_column, protected_attribute]).size()
        if os.environ.get("FPS_UNDERSAMPLE_TARGET"):
            us_target = int(os.environ["FPS_UNDERSAMPLE_TARGET"])
        else:
            q = float(os.environ.get("FPS_UNDERSAMPLE_QUANTILE", "0.5"))
            us_target = int(counts_tmp.quantile(q))
        us_target = max(us_target, knn + 1)
        us_rng = np.random.RandomState(42)
        keep_idx = []
        for _, grp_df in dataset.groupby([class_column, protected_attribute]):
            if len(grp_df) > us_target:
                keep_idx.extend(us_rng.choice(grp_df.index, size=us_target, replace=False))
            else:
                keep_idx.extend(grp_df.index.tolist())
        before_n = len(dataset)
        dataset = dataset.loc[keep_idx].reset_index(drop=True)
        # the fold neighbor cache was built on the full train fold; row positions
        # no longer match after undersampling, so drop it and recompute KNN.
        fold_cache = None
        print(f"[UNDERSAMPLE] target={us_target}/subgroup: {before_n} -> {len(dataset)} rows; "
              f"counts now {dataset.groupby([class_column, protected_attribute]).size().to_dict()}")
    ...
    # === DISJUNCT-AWARE CHANGE: per-(class,protected) small-disjunct deficit =======
    # Mitigation (1): within each minority subgroup's oversampling budget, route a
    # data-driven share toward SMALL-DISJUNCT seeds, sized by how starved that
    # subgroup's protected side is *inside* the small disjuncts (an AOD-oriented
    # target, not a flat count boost). The focus per subgroup is determined here as
    #     m_small[c]   = max over protected p of small-disjunct count in class c
    #     target[c,p]  = aug_disjunct * m_small[c]
    #     deficit[c,p] = max(target[c,p] - small_count[c,p], 0)
    # and capped by the subgroup budget at synthesis time. Gated by FPS_DISJUNCT_AWARE;
    # the in-pocket augmentation rate defaults to `augmentation_rate`, override with
    # FPS_DISJUNCT_AUG. With the gate off this dict is empty and behaviour is baseline.
    disjunct_aware = os.environ.get("FPS_DISJUNCT_AWARE", "0") == "1"
    disjunct_deficit = {}
    small_counts = {}
    if disjunct_aware:
        aug_disjunct = float(os.environ.get("FPS_DISJUNCT_AUG", str(augmentation_rate)))
        small_counts = (dataset[dataset['small_disjunct'] == 1]
                        .groupby([class_column, protected_attribute]).size().to_dict())
        for class_tuple in minority_classes:
            c = class_tuple[0]
            prot_vals = [pt[1] for pt in category_counts if pt[0] == c]
            m_small_c = max((small_counts.get((c, pp), 0) for pp in prot_vals), default=0)
            target = aug_disjunct * m_small_c
            deficit = max(int(round(target)) - small_counts.get(class_tuple, 0), 0)
            disjunct_deficit[class_tuple] = deficit
        print(f"[DISJUNCT] aug_disjunct={aug_disjunct} small_counts={small_counts}")
        print(f"[DISJUNCT] per-subgroup small-disjunct deficit={disjunct_deficit}")
    ...
    if 'single_out' in df_majority.columns:
        ...
        if len(df_majority_single_out) >= (knn+1):
            ...
            replaced_majority = apply_private_smote_replace(
                df_majority.drop(columns=['single_out', 'small_disjunct']),
                epsilon, 
    ...
    for class_tuple, df_subset in df_minority.items():
        ...
        if len(df_single_out)>= (knn+1):
            ...
            replaced = apply_private_smote_replace(
                df_subset.drop(columns=['single_out', 'small_disjunct']),
                epsilon, 
                len(df_single_out), 
                knn, 
                replace=True,
        ...
        # --- Lever 1: cap the per-subgroup oversampling multiple ---------------
        # For a subgroup with very few seeds, demanding ~aug*majority synthetic
        # rows forces a 20-30x blow-up that collapses into a degenerate blob.
        # Cap the target at FPS_CAP_FACTOR x (subgroup seed count) and accept the
        # residual imbalance.  Gated by FPS_CAP_MULTIPLE; only bites tiny groups.
        if os.environ.get("FPS_CAP_MULTIPLE", "0") == "1":
            cap_factor = float(os.environ.get("FPS_CAP_FACTOR", "4"))
            cap = int(cap_factor * len(df_subset))
            if num_samples > cap:
                print(f"[CAP] subgroup {class_tuple}: num_samples {num_samples} -> {cap} "
                      f"(factor {cap_factor} x {len(df_subset)} seeds)")
                num_samples = cap

        # === DISJUNCT-AWARE CHANGE: focused small-disjunct injection (1)+(2) ========
        # Mitigation (1): carve `focused` rows out of this subgroup's budget and
        # generate them from SMALL-DISJUNCT seeds, where `focused` is the within-pocket
        # protected deficit computed in Step 3 (capped by the budget). The remainder
        # falls through to the unchanged augmentation below.
        # Mitigation (2): if the in-group small-disjunct pocket has fewer than knn+1
        # rows, borrow neighbours from the whole same-class pool (any protected value)
        # so interpolation is not a degenerate blob -- seeds stay this subgroup's
        # small-disjunct rows, identity is stamped back afterwards.
        addon_mode = os.environ.get("FPS_DISJUNCT_ADDON", "0") == "1"
        if disjunct_aware and (num_samples > 0 or addon_mode):
            df_small_seed = df_subset[df_subset['small_disjunct'] == 1]
            # ADD-ON-TOP variant (FPS_DISJUNCT_ADDON=1): the focused rows are EXTRA mass
            # (the full small-disjunct deficit, not carved from the budget), so the pocket
            # actually gains density. Default behaviour carves the focus out of the budget.
            focused = (disjunct_deficit.get(class_tuple, 0) if addon_mode
                       else min(disjunct_deficit.get(class_tuple, 0), num_samples))
            pocket = len(df_small_seed)
            if focused > 0 and pocket >= 1:
                need_borrow = pocket < (knn + 1)
                aug_focus = None
                if need_borrow:
                    pool = dataset[dataset[class_column] == class_tuple[0]].copy()
                    if len(pool) >= (knn + 1):
                        pool_seed = pool['small_disjunct'].where(
                            pool[protected_attribute] == class_tuple[1], 0)
                        print(f"[DISJUNCT-BORROW] subgroup {class_tuple}: focused={focused} "
                              f"from {pocket} small-disjunct seeds, neighbours over "
                              f"{len(pool)} same-class rows")
                        aug_focus = apply_private_smote_new(
                            pool.drop(columns=["single_out", "small_disjunct"]),
                            epsilon, focused, False, knn, k, key_vars,
                            pool_seed, precomputed_neighbor_indices=None,
                        )
                        aug_focus[protected_attribute] = class_tuple[1]
                        aug_focus[class_column] = class_tuple[0]
                elif len(df_subset) >= (knn + 1):
                    seed_mask = df_subset['small_disjunct']
                    if os.environ.get("FPS_DISJUNCT_BORDERLINE", "0") == "1":
                        bm = _borderline_seed_mask(df_subset, dataset, class_column, knn)
                        if int(bm.sum()) > 0:
                            seed_mask = bm
                            print(f"[DISJUNCT-BORDERLINE] subgroup {class_tuple}: "
                                  f"{int(bm.sum())}/{pocket} small-disjunct seeds are borderline")
                        else:
                            print(f"[DISJUNCT-BORDERLINE] subgroup {class_tuple}: "
                                  f"no borderline seeds among {pocket}, using all")
                    print(f"[DISJUNCT-FOCUS] subgroup {class_tuple}: focused={focused} "
                          f"from {int(seed_mask.sum())} seeds, neighbours over "
                          f"{len(df_subset)} subgroup rows")
                    aug_focus = apply_private_smote_new(
                        df_subset.drop(columns=["single_out", "small_disjunct"]),
                        epsilon, focused, False, knn, k, key_vars,
                        seed_mask, precomputed_neighbor_indices=None,
                    )
                if aug_focus is not None:
                    if 'highest_risk' in aug_focus.columns:
                        aug_focus = aug_focus.drop(columns=['highest_risk'])
                    if os.environ.get("FPS_DISJUNCT_CLEAN", "0") == "1":
                        before = len(aug_focus)
                        aug_focus = _enn_clean_synthetic(aug_focus, dataset, class_column, knn)
                        print(f"[DISJUNCT-CLEAN] subgroup {class_tuple}: ENN kept "
                              f"{len(aug_focus)}/{before} focused rows")
                    if single_out_only_mode:
                        aug_focus['single_out'] = 0
                    if synthetic_only_mode:
                        aug_focus['synthetic'] = 1
                    generated_data.append(aug_focus)
                    if not addon_mode:
                        num_samples -= len(aug_focus)    # carve the focused share out of the budget
        ...
        if not df_subset.empty and len(df_subset) >= (knn+1) and not df_single_out.empty and num_samples > 0:
            ...
            # --- Lever 2: cross-subgroup neighbor borrowing -------------------
            # For a sub-viable subgroup, interpolating among its own ~6 points
            # yields a degenerate blob.  Instead seed from the subgroup's own
            # single-outs but draw neighbors from the whole same-class pool
            # (any protected value), then stamp the protected attribute back.
            # This injects real geometric spread and moves synthetic points off
            # the few real seeds.  Gated by FPS_BORROW_NEIGHBORS; only fires for
            # subgroups smaller than FPS_BORROW_THRESHOLD.
            borrow = (os.environ.get("FPS_BORROW_NEIGHBORS", "0") == "1"
                      and len(df_subset) < int(os.environ.get("FPS_BORROW_THRESHOLD", "25")))
            if borrow:
                pool = dataset[dataset[class_column] == class_tuple[0]].copy()
                # seeds: only this subgroup's single-outs (others zeroed out)
                pool_single_out = pool['single_out'].where(
                    pool[protected_attribute] == class_tuple[1], 0)
                print(f"[BORROW] subgroup {class_tuple}: seeding from "
                      f"{int((pool[protected_attribute] == class_tuple[1]).sum())} minority rows, "
                      f"neighbors over {len(pool)} same-class rows")
                augmented = apply_private_smote_new(
                    ...
                    key_vars,
                    pool_single_out,
                    precomputed_neighbor_indices=None,  # recompute KNN over the larger pool
                )
                # restore subgroup identity (protected attr interpolated away above)
                augmented[protected_attribute] = class_tuple[1]
                augmented[class_column] = class_tuple[0]
            else:
                subgroup_neighbor_indices = _get_cached_neighbor_indices(
                    fold_cache,
                    (class_tuple[0], class_tuple[1]),
    ...
    if 'small_disjunct' in final_df.columns:        # === DISJUNCT-AWARE CHANGE ===
        final_df = final_df.drop(columns=['small_disjunct'])
