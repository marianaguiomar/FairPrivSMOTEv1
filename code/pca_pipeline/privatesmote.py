...
import argparse
import os
from pipeline_helper import reduce_for_neighbors
...
class PrivateSMOTE:
    ...
    def nearest_neighbours(self, df):
        ...
        self.standardized_data = reduce_for_neighbors(StandardScaler().fit_transform(df))
    ...
    def over_sampling(self, replace, n_samples = None):
        ...
        if replace:
            # If specific indices provided (for single-outs), use only those
            if self.single_out_indices is not None:
                sample_indices = self.single_out_indices
            else:
        ...
        # --- Fix 4: optional latent-space (PCA) interpolation setup ---
        # Interpolate the continuous block on its PCA manifold instead of per-feature
        # in the original space, then inverse-transform. Binary columns keep the
        # original per-feature scheme. Gated by FPS_PCA_INTERPOLATE; all-numeric only.
        self.latent_mode = (
            os.environ.get("FPS_PCA_INTERPOLATE", "0") == "1"
            and not np.any(self.is_object_type)
        )
        if self.latent_mode:
            n_unique = [len(np.unique(self.x[:, j])) for j in range(self.x.shape[1])]
            self.cont_mask = np.array([n > 2 for n in n_unique])
            if int(self.cont_mask.sum()) >= 3:
                Xc = self.x[:, self.cont_mask].astype(float)
                self.scaler_l = StandardScaler().fit(Xc)
                variance = float(os.environ.get("FPS_PCA_VARIANCE", "0.95"))
                self.pca_l = PCA(n_components=variance, svd_solver="full", random_state=42)
                self.Z = self.pca_l.fit_transform(self.scaler_l.transform(Xc))
                self.z_std = self.Z.std(axis=0)
                self.cont_idx = np.where(self.cont_mask)[0]
            else:
    ...
    def _populate(self, N, i, nnarray):
        ...
        while N != 0:
            ...
            # --- Fix 4: overwrite continuous columns with latent-space interpolation ---
            if getattr(self, "latent_mode", False):
                zi = self.Z[i]
                zn = self.Z[nnarray[neighbour]]
                lap = np.random.laplace(0, 1 / self.epsilon, size=zi.shape)
                z_new = np.where(
                    zn == zi,
                    zi + np.random.choice([-1, 1], size=zi.shape) * self.z_std * lap,
                    zi + (zn - zi) * lap,
                )
                x_cont = self.scaler_l.inverse_transform(
                    self.pca_l.inverse_transform(z_new.reshape(1, -1))
                )[0]
                for kk, j in enumerate(self.cont_idx):
                    val = x_cont[kk]
                    # clip to the original per-feature range, like the original scheme
                    val = min(max(val, self.min_values[j]), self.max_values[j])
                    new_nums_values[j] = np.array([val], dtype=float)
...
def apply_private_smote_replace(data, epsilon, n_samples, knn, replace, single_out_indices=None, precomputed_neighbor_indices=None):
    ...
    - single_out_indices (list): Specific indices to replace (for fitting KNN on full data but replacing subset).
