...
input_folder_name = "23"
final_folder_name = "23_pca"
method_number = "3"
...
# --- PCA neighbor strategy (Fix 1) -----------------------------------------
# Project the standardized features into a PCA subspace before the kNN search
# that seeds PrivateSMOTE interpolation. Read at runtime by
# pipeline_helper.reduce_for_neighbors (used in build_fold_subgroup_cache and
# the nearest_neighbours fallbacks). Toggle here for the full experiment.
use_pca_neighbors = True   # False -> original full-dimensional neighbor search
pca_variance = 0.95        # retained-variance ratio (standard default)
os.environ["FPS_PCA_NEIGHBORS"] = "1" if use_pca_neighbors else "0"
os.environ["FPS_PCA_VARIANCE"] = str(pca_variance)
os.environ["FPS_PCA_INTERPOLATE"] = "0"  # Fix 4 (latent interpolation); off for Fix 1
# ---------------------------------------------------------------------------
