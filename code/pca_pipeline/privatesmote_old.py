...
class newPrivateSMOTE:
    """Apply PrivateSMOTE"""

    def __init__(self, samples, knn, epsilon, k, key_vars, single_out_column, precomputed_neighbor_indices=None):
        """Initiate arguments"""
        ...
        self.X_train_shape = ()
        self.x = ()
        self.single_out_column = single_out_column
    ...
    def over_sampling(self, desired_synthetic_count):
        ...
        if desired_synthetic_count <= len(highest_risk_indices):
            selected_indices = np.random.choice(highest_risk_indices, size=desired_synthetic_count, replace=False)
        else:
            selected_indices = np.random.choice(highest_risk_indices, size=desired_synthetic_count, replace=True)


        # For each observation find nearest neighbours
        # Generate one synthetic sample per selected index
        for i in selected_indices:
            if self.precomputed_neighbor_indices is not None:
                nnarray = self.precomputed_neighbor_indices[i]
            else:
                nnarray = self.neighbors.kneighbors(self.standardized_data[i].reshape(1, -1), return_distance=False)[0]
...
def apply_private_smote_new(data, epsilon, n_samples, replace, knn, k, key_vars, single_out_column, precomputed_neighbor_indices=None):
    """
...
def apply_original_private_smote(data, dataset_name, knn, per, epsilon, k, key_vars, output_folder, nqi):
    ...
    # Apply PrivateSMOTE
    newDf = PrivateSMOTE(data, per, knn, epsilon,
                         k, key_vars).over_sampling()
    ...
    newDf = newDf.drop(columns=['highest_risk'])
    ...
    file_name = f'{dataset_name}_eps{epsilon}_k{k}_knn{knn}_per{per}_privateSMOTE_QI{nqi}.csv'
...
if __name__ == "__main__":
    ...
    # Create the file name as before
    file_name = f'{args.dataset_name}_eps{args.epsilon}_k{args.k}_knn{args.knn}_per{args.per}_privateSMOTE_QI{args.nqi}.csv'
