...
# Inference risk is stored as one column per sensitive attribute (NaN-padded when a
# dataset has fewer than three). We pool all of them per dataset.
...
def _read_inference_data(dataset_dir: Path, mode: str = "median") -> float:
    csv_frames = []
    ...
    for csv_path in sorted(dataset_dir.rglob("*.csv")):
        ...
        if cols:
            csv_frames.append(frame[cols].copy())

    if not csv_frames:
        return np.nan

    combined = pd.concat(csv_frames, ignore_index=True)
    # Pool every per-sensitive-attribute value across folds; stack() drops the NaN pads.
    values = pd.to_numeric(combined.stack(), errors="coerce")
...
def run_analysis(mode: str, use_log: bool, scope: str):
    ...
    print(f"Fetching inference data ({mode}) for scope: {scope}...")

    eps_original_list = []
    fp_original_list = []

    for dataset in target_datasets:
        eps_dir = INFERENCE_METHODS["epsilon-PrivateSMOTE"] / dataset
        eps_val = _read_inference_data(eps_dir, mode)
        eps_original_list.append(eps_val if not pd.isna(eps_val) else 0.0)

        fp_dir = INFERENCE_METHODS["FP-SMOTE"] / dataset
        fp_val = _read_inference_data(fp_dir, mode)
    ...
    print(f"\n--- Inference Data ({scope.upper()}, {mode.upper()}) ---")
    ...
    plt.title(f"Mean Inference Risk by Dataset", size=14, y=1.1)
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))

    plt.tight_layout()
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = OUTPUT_DIR / "radar_inference.png"
