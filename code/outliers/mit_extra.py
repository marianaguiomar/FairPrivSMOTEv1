"""
Generate the outlier-aware mitigation (variant A) for the two datasets the main
pipeline skips (55=BPIC, adult), so the A-vs-baseline table can cover all 13.

Matched to the stored `outlier_pipeline_napierala` run: Napierala 'Outlier' mask,
DEISOLATE=True (default), grid eps{0.1,0.5} x k{3,5} x knn5 x aug0.4, all QI sets,
binning=None, removal=None, qi_only_visualization=True, RF seed 57, folds
StratifiedKFold(5, shuffle, random_state=42). Writes to final folder `mit_55_adult`.

    priv39/bin/python code/outliers/mit_extra.py
"""
...
DATASETS = ["55", "adult"]          # smaller first
