"""
Finish variant C cheaply: generate the REMAINING datasets at FOLD 1 ONLY, with the
faithful two-stage de-isolation (DEISO_MODE='twostage'), APPENDING to the existing
`outlier_pipeline_faithful` folder (which already holds 13/23/3/33/37/compas at 5 folds).

Same matched settings as gen_faithful_deiso.py; only difference is fold_idx==0 only.

    priv39/bin/python code/outliers/gen_faithful_rest.py
"""
import os, sys
import numpy as np
import pandas as pd
...
DATASETS = ["credit", "german", "law", "oulad", "student"]   # the ones not yet done
...
def main():
    ...
    for ds in DATASETS:
        ...
        print(f"\n=== {ds} ({len(data)} rows, {len(key_vars)} QI sets) FOLD1 ONLY ===", flush=True)
