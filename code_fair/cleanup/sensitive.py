...
def calculate_target_changes(X, y, protected_attr_idx):
    ...
    for index, row in X_test.iterrows():
        row_ = [row.values]  # Select the row values
        
        y_normal = clf.predict(row_)
        
        # Flip the protected attribute (protected_attr_idx)
        row_[0][protected_attr_idx] = 1 - row_[0][protected_attr_idx]  # Flip the value of the protected attribute
